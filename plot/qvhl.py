...
import sys
sys.path = ['/mnt/d/UniVTG-main/UniVTG-main'] + sys.path
#print(sys.path)
...
def plot_video(pred_json, gt_json, save_dir_i, fig_num=None, template=True):
    ...
    #print(vid_exists)
    #print(os.path.join(video_path, pred_json['vid'] + '.mp4'))
    if vid_exists:
        ...
        for i, t in enumerate(np.linspace( t_min, t_max, fig_num)):
            ...
            #print(f"original pixel: {frame}")
            #print(f"original pixel dumped: {cv2.utils.dumpInputArray(frame)}")
    ...
    _, _, text_width1, text_height1 = draw.textbbox((0, 0), text1, font=font1)
    _, _, text_width2, text_height2 = draw.textbbox((0, 0), text2, font=font2)
...
def plot_mr(pred_json, gt_json, save_dir_i, only_one_gt=False, pred_num=1, base_json=None):
    ...
    if not base_json:
        ...
        ax.barh(["VTG using CAT Model's Prediction", "GT Interval"], [gt_json['duration'], gt_json['duration']], color="white", edgecolor="black", height=0.6, left=0, linewidth=2)
        ...
        for i in range(len(pred_json['pred_relevant_windows'][:pred_num])):
            ...
            ax.barh("VTG using CAT Model's Prediction", interval[1] - interval[0], color=color2, edgecolor=color2_dark, height=0.6, left=interval[0], linewidth=2)
        ...
        ax.text(0, 0, "    VTG using CAT Model's Prediction", va='center', ha='left', color=color2_dark, fontproperties=font_prop3)
        ...
        ax.barh(["MomentDETR's Prediction", "VTG using CAT Model's Prediction", "GT Interval"], [gt_json['duration'], gt_json['duration'], gt_json['duration']], color="white", edgecolor="black", height=0.6, left=0, linewidth=2)
        ...
        ax.text(0, 1, "    VTG using CAT Model's Prediction", va='center', ha='left', color=color2_dark, fontproperties=font_prop3)
...
def plot_hl(pred_json, gt_json, save_dir_i,  base_json=None):
    ...
    if not base_json:
        ...
        plt.plot(x, gt_saliency, label='GT Saliency', color=color1_dark, linewidth=6, linestyle='solid')
        plt.plot(x, pred_saliency, label='VTG using CAT Model\'s Prediction', color=color2_dark, linewidth=6, linestyle='solid')
...
if __name__ == "__main__":
    ...
    # only_one_gt = Falsepwd    
    # pred_num = 1
    only_one_gt = False
    pred_num = None
    
    # load prediction, ground truth, and video
    video_path="/mnt/d/UniVTG-main/UniVTG-main/examples"
    gt_path="./plot/qvhl/gt.jsonl"

    dset_path="/data/qvhighlights"
    checkpoint_dir="./plot/qvhl"
    json_file="univtg.jsonl"
    
    # baseline
    base_checkpoint_dir = "/plot/qvhl"
    base_json_file = "./plot/qvhl/momentdetr.jsonl"
    
    # pred_json_val = load_jsonl(os.path.join(dset_path, checkpoint_dir, json_file))
    pred_json_val = load_jsonl(os.path.join(checkpoint_dir, json_file))
    gt_json_val = load_jsonl(gt_path)
    
    # base_json_val = load_jsonl(os.path.join(base_checkpoint_dir, base_json_file))
    base_json_val = load_jsonl(os.path.join( base_json_file))
    # base_json_val = None
